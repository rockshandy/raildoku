...
module LocalSearch
		  ...
		  for row in 0...board.length()
			  ...
			  for collumn in 0...board[0].length()
				  if board[row][collumn].is_a?(Array)#if it hasn't been assigned a value
					  ...
					  #p queue[0][row][collumn]
					  checkBoard[row][collumn]=board[row][collumn];
				  end
			  end
		  end
		  boardHash=create_node_consistent_board(checkBoard,Math.sqrt(checkBoard.size()).floor,Math.sqrt(checkBoard[0].size()).floor)
		  #p "::"
		  #p checkBoard
		  #gets
		  if boardHash==false
			  ...
			  for row in 0...board.length()
				  for collumn in 0...board[0].length()
					  if !board[row][collumn].is_a?(Array)
						  ...
						  #p collumn
						  #p board[row][collumn]
							  if !(cellToComp[:values].is_a?(Array)) and cellToComp[:values]==board[row][collumn] and !(cellToComp[:x]==row and cellToComp[:y]==collumn)
								  #duplicate value
								  goodBoard=false
								  #break;
							  elsif cellToComp[:values].is_a?(Array) and cellToComp.length()==1 and cellToComp[:values][0]==board[row][collumn] and !(cellToComp[:x]==row and cellToComp[:y]==collumn)
								  #duplicate value
								  goodBoard=false
								  #break;
							  end
						  ...
						  uniqueLocalCells(board,row,collumn).each{|cellToComp|
						  #p cellToComp
							  if !(cellToComp[:values].is_a?(Array)) and cellToComp[:values]==board[row][collumn][0] and !(cellToComp[:x]==row and cellToComp[:y]==collumn)
								  #duplicate value
								  goodBoard=false
								  #break;
							  elsif cellToComp[:values].is_a?(Array) and cellToComp.length()==1 and cellToComp[:values][0]==board[row][collumn][0] and !(cellToComp[:x]==row and cellToComp[:y]==collumn)
								  #duplicate value
								  goodBoard=false
								  #break;
							  end
  ...
  def localSearch(board,heuristic)
	  solutionFound=false;
	  queue=Array.new(1,board);
	  checkBoard=Array.new()
	  for row in 0...queue[0].length()
		  ...
		  for collumn in 0...queue[0][0].length()
			  if queue[0][row][collumn].is_a?(Array)#if it hasn't been assigned a value
				  if queue[0][row][collumn].length()==1
					  #checkBoard[row][collumn]=queue[0][row][collumn][0];#it only has one option so it will be assigned
	  ...
	  if (create_node_consistent_board(checkBoard,Math.sqrt(checkBoard.size()).floor,Math.sqrt(checkBoard[0].size()).floor)[1].empty?)
		  solutionFound=true;
	  else
		  queue=Array.new(1,create_node_consistent_board(checkBoard,Math.sqrt(checkBoard.size()).floor,Math.sqrt(checkBoard[0].size()).floor)[0])
	  #print_board(queue)
	  #gets
	  end
	  while !solutionFound
		  ...
		  if queue.include?(newBoard)#if its a valid board re-add it now that the last board's possibilities have been updated
			  ...
			  for row in 0...newBoard.length()
				  for collumn in 0...newBoard[0].length()
					  if newBoard[row][collumn].is_a?(Array)#if it hasn't been assigned a value
						  if newBoard[row][collumn].length()==1
							  #newBoard[row][collumn]=newBoard[row][collumn][0];#it only has one option so it will be assigned
							  newBoard[row][collumn]=0;#do not assign right now to keep track of changes
						  else
							  newBoard[row][collumn]=0;
						  end
					  else
			  ...
			  #generate new board
			  myboardHash=create_node_consistent_board(newBoard,Math.sqrt(newBoard.length()).floor,Math.sqrt(newBoard[0].length()).floor);
			  if myboardHash!=false
				  for row in 0...myboardHash[0].length()
					  for collumn in 0...myboardHash[0][0].length()
						  if myboardHash[0][row][collumn].is_a?(Array) and queue[1][row][collumn].is_a?(Array) and myboardHash[0][row][collumn].length()>queue[1][row][collumn].length()
		  ...
		  if (boardHash!=false and boardHash[1].empty?)
			  ...
			  while same && queue.length>0
				  # for row in 0...queue[0].length()
					  # for collumn in 0...queue[0][0].length()
						  # if queue[0][row][collumn].is_a?(Array) and checkBoard[row][collumn].is_a?(Array)
							  # if queue[0][row][collumn].length()!=checkBoard[row][collumn].length()
								  # same=false;
							  # else
								  # for entry in 0...queue[0][row][collumn].length()
									  # if queue[0][row][collumn][entry]!=checkBoard[row][collumn][entry]
										  # same=false;
									  # end
								  # end
							  # end
						  # elsif !queue[0][row][collumn].is_a?(Array) and checkBoard[row][collumn].is_a?(Array) and checkBoard[row][collumn].length()==1
							  # if queue[0][row][collumn]!=checkBoard[row][collumn][0]
								  # same=false;
							  # end
						  # else
							  # same=false;
...
# can use an as example to run itself but commented out in rails
def example
#include LocalSearch
...
#p create_node_consistent_board(board,Math.sqrt(board.length).floor,Math.sqrt(board[0].length).floor);
#localSearch(create_node_consistent_board(board,Math.sqrt(board.length).floor,Math.sqrt(board[0].length).floor)[0],'minConflicts');
print_board (localSearch(create_node_consistent_board(board,Math.sqrt(board.length).floor,Math.sqrt(board[0].length).floor)[0],'mostHighlyConstrainedVariableWithLeastConstrainingValue'));

#board=[[9, 5, 5, 4, 5, 7, 6, 1, 5], [2, 1, 7, 8, 5, 5, 5, 4, 3], [4, 5, 5, 1, 2, 6, 5, 7, 5], [1, 2, 8, 9, 4, 3, 7, 5, 6], [7, 6, 9, 5, 1, 5, 3, 3, 4], [3, 4, 5, 7, 6, 5, 3, 8, 9], [3, 3, 2, 6, 7, 1, 3, 3, 5], [6, 7, 1, 5, 5, 5, 9, 3, 5], [3, 9,4, 5, 5, 8, 1, 6, 2]]

#p create_node_consistent_board(board,Math.sqrt(board.length).floor,Math.sqrt(board[0].length).floor)

#board =[[9, 1, 8, 4, 3, 5, 6, 7, 8], [2, 5, 7, 9, 8, 6, 4, 1, 3], [4, 6, 0, 1, 2, 7, 5, 9, 5], [1, 2, 9, 8, 4, 3, 7, 5, 6], [7, 8, 6, 5, 1, 9, 2, 2, 4], [3, 4, 5, 7, 6, 2, 3, 8, 9], [8, 9, 2, 6, 7, 1, 5, 4, 5], [6, 3, 1, 2, 5, 4, 9, 3, 7], [5, 7, 4, 3, 9, 8, 1, 6, 2]];
#if create_node_consistent_board(board,Math.sqrt(board.length).floor,Math.sqrt(board[0].length).floor) ==false
#	p "bad"
#end

#create_node_consistent_board(checkBoard,Math.sqrt(checkBoard.size()).floor,Math.sqrt(checkBoard[0].size()).floor)[1].empty?
end
